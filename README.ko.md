...
턴별 토큰 분석, 네 가지 가격 티어에 걸친 비용, TTL 믹스와 적중률이 있는 캐시 분석, 모든 세션에 겹쳐진 품질 점수, 서브에이전트 비용 분석, 측정됨과 추정을 분리해 유지하는 절감 추적기. 설치 후 설정 불필요. [전체 대시보드 문서 →](https://alexgreensh.github.io/token-optimizer/features/dashboard/)

## 절감 내역

여섯 가지 레버, 모두 자동: 도구 출력 압축, 턴 간 중복 제거, 델타 읽기, 구조 맵 스켈레톤, 체크포인트 복원, 모델 라우팅. 이들의 절감은 세 개의 계층으로 추적됩니다.

**세 개의 계층, 각각 파악 방식으로 표시** (실제 한 달: 저자의 최근 30일, 여섯 가지 레버 모두 켜짐):

- **기록된 행동(\~$126)**, 측정됨. 모든 절감을 발생 시점에 기록: 결과 보관, 반복 읽기 건너뛰기, 간결한 재개, 루프 조기 중단. 전후 토큰 수가 있는 2,733개 이벤트.
- **회피한 반복 읽기(\~$1,124)**, 추정. Token Optimizer가 제거한 내용은 해당 세션의 다음 실제 컴팩션까지 이후 모든 턴에서 다시 읽혔을 내용입니다. 각 턴의 캐시 읽기 요율로 턴별로 모델링하고 실제 컴팩션 이벤트를 상한으로 둡니다.
- **기타 추정(\~$146)**, 추정. 간결한 세션 재개, 압축된 서브에이전트 컨텍스트, MCP 출력 상한, 방지된 루프.

이번 달 API 환산 가치 약 **\~$1,396**, 그리고 **5,960만 토큰이 모델에 전송되지 않음**, 워크로드의 약 28% (1억 5,080만 사용, Token Optimizer 없이는 약 2억 1,040만). 토큰이 기본 지표이고 달러는 사용하는 모델에 따라 달라집니다. 측정됨과 추정은 절대 하나의 숫자로 합치지 않습니다. [전체 방법론 보기 →](BENCHMARK.md)

<p align="center">
  <img src="skills/token-optimizer/assets/real-savings.svg" alt="30일 절감 보고서: API 환산 약 $1,396, 5,960만 토큰 미전송. 기록된 행동 $126(측정됨), 회피한 반복 읽기 $1,124(추정), 기타 추정 $146으로 구분" width="900">
</p>

최근 30일간 1,698개 세션 기반(스냅샷 2026-09-14). 당신의 숫자는 당신만의 것입니다: 대시보드가 당신의 세션으로 같은 보고서를 만듭니다. [방법론 보기 →](BENCHMARK.md)
...
**PolyForm Noncommercial 1.0.0**, 그리고 서면으로 된 [소규모 사업자 허가](LICENSE-SMALL-BUSINESS.md). 소스 공개.

### 개인 / 취미 / 연구 / 교육?
하세요. 전체 소스, 로컬 실행, 라이선스 구매 불필요. 개인 오픈소스 사이드 프로젝트도 포함됩니다. 기준은 어디서 실행하느냐가 아니라 누구를 위한 작업이냐입니다. 회사를 위한 작업은 집에서 하더라도 회사의 사용입니다.

### 소규모 사업자 (5인 미만 그리고 월 매출 $20k 미만)?
무료 상업 라이선스를 서면으로 제공합니다: [LICENSE-SMALL-BUSINESS.md](LICENSE-SMALL-BUSINESS.md). 두 조건 모두 팀이나 사용자 수가 아니라 **회사 전체**(모회사·계열사 포함)에 적용됩니다. 큰 회사 안의 소규모 팀은 상업 라이선스가 필요합니다.

### 개인으로 시작했는데, 이제 비즈니스로 변하고 있나요?
과거 사용은 완전히 괜찮습니다. 조건을 벗어난 뒤 32일의 유예 기간 동안 상업 라이선스를 준비하면 됩니다.

### 더 큰 회사 / 상업적 사용?
[Alex Greenshpun](https://linkedin.com/in/alexgreensh) 또는 me@alexgreenshpun.com에 연락하세요. 조건은 합리적이며 회사 규모에 맞춰 정해집니다.
