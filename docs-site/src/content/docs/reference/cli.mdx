...
| `antigravity-install` | Install the Antigravity plugin into `~/.gemini/config/plugins/token-optimizer/` | n/a |
| `antigravity-uninstall` | Remove the Antigravity plugin (only what was installed) | n/a |
...
The default dashboard port is 24842 for Claude Code, 24843 for Codex, 24844 for Hermes, 24845 for Copilot, 24846 for Cursor, and 24847 for Antigravity. The daemon serves a bookmarkable URL at `http://localhost:<port>/token-optimizer`; without the daemon, the dashboard opens from a generated file.
...
| `route` | Recommend a model and reasoning effort for one task, in the current platform's own model names and effort knob | `--task "..."`, `--json` |
...
| Command | Description | Key flags |
|---|---|---|
| `antigravity-doctor` | Check Antigravity readiness (home, plugin, consent, `agy` binary, store readability, dashboard port) | `--json` |
| `antigravity-summary` | Token and cost summary for recent Antigravity conversations | n/a |
| `antigravity-rollup` | Ingest recent Antigravity conversations into the history database. Runs automatically via the Stop hook. | `--quiet` |
| `antigravity-home` | Print the resolved Antigravity home (override via `TOKEN_OPTIMIZER_ANTIGRAVITY_HOME`) | n/a |
