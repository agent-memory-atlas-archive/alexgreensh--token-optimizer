...
ターンごとのトークン内訳、4つの価格ティアにわたるコスト、TTLミックスとヒット率を伴うキャッシュ分析、すべてのセッションに重ねられた品質スコア、サブエージェントコスト内訳、計測済みと推定を分けて保持する節約トラッカー。インストール後ゼロ設定。[完全なダッシュボードドキュメント →](https://alexgreensh.github.io/token-optimizer/features/dashboard/)

## 何を節約するか

6つのレバー、すべて自動: ツール出力圧縮、ターン間の重複排除、差分読込、構造マップのスケルトン、チェックポイント復元、モデルルーティング。その節約は3つの階層で追跡されます。

**3つの階層、それぞれの把握方法でラベル付け**(実際の1か月: 著者の直近30日間、6つのレバーすべてがオン):

- **記録されたアクション(\~$126)**、計測済み。すべての節約を発生時に記録: 結果のアーカイブ、繰り返し読込のスキップ、リーンな再開、ループの早期停止。前後のトークン数付きで2,733イベント。
- **回避された繰り返し読込(\~$1,124)**、推定。Token Optimizerが削除した内容は、そのセッションの次の実際のコンパクションまで後続のすべてのターンで再読込されていたはずのもの。各ターンのキャッシュ読込レートでターンごとにモデル化し、実際のコンパクションイベントを上限とする。
- **その他の推定(\~$146)**、推定。リーンなセッション再開、圧縮されたサブエージェントのコンテキスト、MCP出力の上限、防止されたループ。

この月のAPI換算価値は約 **\~$1,396**、そして **5,960万トークンがモデルに送信されず**、ワークロードの約28%(消費1億5,080万、Token Optimizerなしなら約2億1,040万)。トークンが主要な指標で、ドルは使用するモデルに依存します。計測済みと推定は決してひとつの数字にまとめません。[完全な方法論を見る →](BENCHMARK.md)

<p align="center">
  <img src="skills/token-optimizer/assets/real-savings.svg" alt="30日節約レポート: API換算で約$1,396、5,960万トークン未送信。内訳は記録されたアクション$126(計測済み)、回避された繰り返し読込$1,124(推定)、その他の推定$146" width="900">
</p>

直近30日間の1,698セッションに基づく(スナップショット2026-09-14)。あなたの数字はあなた自身のものです: ダッシュボードがあなたのセッションから同じレポートを生成します。[方法論を見る →](BENCHMARK.md)
...
**PolyForm Noncommercial 1.0.0** に加え、書面による[スモールビジネス許諾](LICENSE-SMALL-BUSINESS.md)があります。ソース公開。

### 個人 / ホビー / 研究 / 教育?
どうぞ。完全なソース、ローカルで動作、ライセンス購入不要。ご自身のオープンソースのサイドプロジェクトも含みます。基準は「どこで動かすか」ではなく「誰のための作業か」です。勤務先のための作業は、自宅で行っても勤務先の利用になります。

### スモールビジネス(5人未満 かつ 月の売上$20k未満)?
無償の商用ライセンスを書面で付与しています: [LICENSE-SMALL-BUSINESS.md](LICENSE-SMALL-BUSINESS.md)。どちらの条件も、チームや利用者数ではなく**会社全体**(親会社・グループ会社を含む)に適用されます。大きな会社の中の少人数チームには商用ライセンスが必要です。

### 個人で始めたが、ビジネスになりつつある?
過去の使用は全く問題ありません。条件を満たさなくなってから32日間の猶予があるので、その間に商用ライセンスを手配してください。

### より大きな会社 / 商用利用?
[Alex Greenshpun](https://linkedin.com/in/alexgreensh)またはme@alexgreenshpun.comに連絡してください。条件は妥当で、会社の規模に応じて決まります。
