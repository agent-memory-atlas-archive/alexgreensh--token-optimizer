...
每轮 token 分解、跨四个定价层级的成本、带 TTL 混合和命中率的缓存分析、叠加在每个会话上的质量评分、子代理成本分解、已计量与估算分开的节省追踪器。安装后零设置。[完整仪表盘文档 →](https://alexgreensh.github.io/token-optimizer/features/dashboard/)

## 节省什么

六个杠杆,全部自动:工具输出压缩、跨轮去重、增量读取、结构图骨架、检查点恢复和模型路由。它们的节省按三个层级追踪。

**三个层级,按各自的认定方式标注**(真实的一个月:作者最近 30 天,六个杠杆全开):

- **已记录的动作(\~$126)**,已计量。每次节省都在发生时记录:归档一个结果、跳过一次重复读取、精简恢复、提前终止循环。共 2,733 个事件,均带有前后 token 数。
- **避免的重复读取(\~$1,124)**,估算。Token Optimizer 移除的内容本会在之后每一轮被重复读取,直到该会话下一次真正的压缩;按每轮的缓存读取费率逐轮建模,并以真实压缩事件为上限。
- **其他估算(\~$146)**,估算。精简会话恢复、压缩的子代理上下文、MCP 输出上限、被阻止的循环。

本月约 **\~$1,396** 的 API 等值价值,以及 **5,960 万个从未发送给模型的 token**,约占工作量的 28%(已花费 1.508 亿,若无 Token Optimizer 约需 2.104 亿)。token 是主要指标;美元取决于你使用的模型。已计量与估算从不混为一个数字。[查看完整方法学 →](BENCHMARK.md)

<p align="center">
  <img src="skills/token-optimizer/assets/real-savings.svg" alt="30 天节省报告:约 $1,396 API 等值价值,5,960 万 token 未发送;分为已记录动作 $126(已计量)、避免的重复读取 $1,124(估算)和其他估算 $146" width="900">
</p>

基于最近 30 天的 1,698 个会话(快照 2026-09-14)。你的数字是你自己的:仪表板会从你自己的会话生成同样的报告。[查看方法学 →](BENCHMARK.md)
